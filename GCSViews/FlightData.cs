...
namespace MissionPlanner.GCSViews
{
    public partial class FlightData : MyUserControl, IActivate, IDeactivate
    {
            ...
            for (int f = 1; f < 9; f++) //f=30 de base
            {
                ...
                if (Settings.Instance["quickView" + f] != null)
                {
                    ...
                    if (ctls.Length > 0)
                    {
                        ...
                        #region nom quickviews et databindings
                        if (QV == quickView1)
                            QV.desc = "Tension bat (V)";    //tension batterie
                        if (QV == quickView2)
                            QV.desc = "% Batterie";         //% batterie
                        if (QV == quickView3)
                            QV.desc = "Courant bat (A)";    //Courant Batterie
                        if (QV == quickView4)
                            QV.desc = "Vitesse (nds)";      //Vitesse
                        if (QV == quickView5)
                            QV.desc = "Hauteur d'eau (m)";  //Hauteur d'eau
                        if (QV == quickView6)
                            QV.desc = "Dist Obstacle (m)";  //Distance Obstacle
                        if (QV == quickView7)
                            QV.desc = "Long Cable (m)";     //Longeur cable
                        if (QV == quickView8)
                            QV.desc = "Temp process (°C)";  //température processeur
                        #endregion

                        // set databinding for value
                        #region anciens quickviews
                        //QV.DataBindings.Clear();
                        //try
                        //{
            ...
            while (threadrun)
            {
                ...
                try
                {
                    ...
                    if (MainV2.comPort.MAV.cs.battery_voltage <= critvolt)
                    {
                        ...
                        if (first_time_under_voltage_limit)
                        {
                            date_debut_pb_voltage = DateTime.Now;
                            first_time_under_voltage_limit = false;
                        }

                        if (DateTime.Now - date_debut_pb_voltage < TimeSpan.FromMilliseconds(500))
                        {
                            quickView1.numberColor = quickView1.BackColor;
                        }
                        else
                        {
                            quickView1.numberColor = System.Drawing.Color.FromArgb(((int)(((byte)(255)))), ((int)(((byte)(0)))), ((int)(((byte)(0)))));
                            if (DateTime.Now - date_debut_pb_voltage > TimeSpan.FromMilliseconds(1000))
                                date_debut_pb_voltage = DateTime.Now;
                        }
                    ...
                    if ((MainV2.comPort.MAV.cs.battery_remaining) <= critpercent)
                    {
                        ...
                        if (first_time_under_percent_limit)
                        {
                            ...
                            first_time_under_percent_limit = false;
                        }

                        if (DateTime.Now - date_debut < TimeSpan.FromMilliseconds(500))
                        {
                            quickView2.numberColor = quickView2.BackColor;
                        }
                        else
                        {
                            quickView2.numberColor = System.Drawing.Color.FromArgb(((int)(((byte)(255)))), ((int)(((byte)(0)))), ((int)(((byte)(0)))));
                            if (DateTime.Now - date_debut > TimeSpan.FromMilliseconds(1000))
                                date_debut = DateTime.Now;
                        }
                    } //quickview2 rouge clignotant 
                    else
                    {
        ...
        bool first_alerte_niveau_eau = true;
        DateTime date_debut_pb_water_level = DateTime.Now;
        public void pb_niveau_eau()
        {
            ...
            if (DateTime.Now - date_debut_pb_water_level < TimeSpan.FromMilliseconds(500))
            {
                quickView5.numberColor = quickView5.BackColor;
            }
            else
            {
                quickView5.numberColor = System.Drawing.Color.FromArgb(((int)(((byte)(255)))), ((int)(((byte)(0)))), ((int)(((byte)(0)))));
