...
namespace MissionPlanner.GCSViews
{
    public partial class FlightData : MyUserControl, IActivate, IDeactivate
    {
        ...
        private FilterInfoCollection videoDevices; // Mevcut kameraları tutar
        private VideoCaptureDevice videoSource; // Seçilen kamerayı tutar
        ...
        void chk_box_tunningCheckedChanged(object sender, EventArgs e)
        {
            ...
            {
                ((CheckBox)sender).BackColor = Color.DarkBlue;
        ...
        void chk_box_hud_UserItem_CheckedChanged(object sender, EventArgs e)
        {
            ...
            {
                checkbox.BackColor = Color.DarkBlue;
        ...
        private void StartCamera()
        {
            ...
            MessageBox.Show("Bulunan kamera sayısı: " + videoDevices.Count);
        ...
        public void DrawGeofence(PointLatLng center, double radiusInMeters)
        {

            List<PointLatLng> points = GetCirclePoints(center, radiusInMeters);
            GMapPolygon circle = new GMapPolygon(points, "geofence")
            {
            ...
            geofence.Polygons.Add(circle);


            gMapControl1.Refresh();
        }

        public List<PointLatLng> GetCirclePoints(PointLatLng center, double radius)
        {
            ...
            for (int i = 0; i < segments; i++)
            {
                ...
                double lat = center.Lat + (radius / 111320) * Math.Cos(angle);
                double lng = center.Lng + (radius / (111320 * Math.Cos(center.Lat))) * Math.Sin(angle);
        ...
        private async void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    string response = await client.GetStringAsync("http://127.0.0.1:8000/hss_koordinatlari");
                    JObject json = JObject.Parse(response);
                    JArray jsonArray = (JArray)json["hss_koordinat_bilgileri"];

                    geofence.Polygons.Clear(); // Önceki çemberleri sil

                    foreach (JObject item in jsonArray)
                    {
                        double hssEnlem = item["hssEnlem"].ToObject<double>();
                        double hssBoylam = item["hssBoylam"].ToObject<double>();
                        double hssYaricap = item["hssYaricap"].ToObject<double>();

                        PointLatLng center = new PointLatLng(hssEnlem, hssBoylam);
                        DrawGeofence(center, hssYaricap);
                    }
