...
namespace MissionPlanner.Controls
{
    public partial class ServoOptions : UserControl
    {
        ...
        private void BUT_Low_Click(object sender, EventArgs e)
        {
            try
            {
                if (MainV2.comPort.doCommand(MainV2.comPort.sysidcurrent, (byte)MainV2.comPort.compidcurrent, MAVLink.MAV_CMD.DO_SET_SERVO, thisservo, int.Parse(TXT_pwm_low.Text), 0, 0,
                    0, 0, 0))
                {
        ...
        private void BUT_High_Click(object sender, EventArgs e)
        {
            try
            {
                if (MainV2.comPort.doCommand(MainV2.comPort.sysidcurrent, (byte)MainV2.comPort.compidcurrent, MAVLink.MAV_CMD.DO_SET_SERVO, thisservo, int.Parse(TXT_pwm_high.Text), 0, 0,
                    0, 0, 0))
                {
        ...
        private void BUT_Repeat_Click(object sender, EventArgs e)
        {
            ...
            // MainV2.comPort.doCommand(MainV2.comPort.sysidcurrent, (byte)MainV2.comPort.compidcurrent, MAVLink.MAV_CMD.DO_SET_SERVO, int.Parse(TXT_rcchannel.Text), int.Parse(TXT_pwm_high.Text), 10, 1000, 0, 0, 0);
        }
        ...
        private void But_mid_Click(object sender, EventArgs e)
        {
            try
            {
                if (MainV2.comPort.doCommand(MainV2.comPort.sysidcurrent, (byte)MainV2.comPort.compidcurrent, MAVLink.MAV_CMD.DO_SET_SERVO, thisservo, (int.Parse(TXT_pwm_high.Text) - int.Parse(TXT_pwm_low.Text)) / 2 + int.Parse(TXT_pwm_low.Text), 0, 0,
                    0, 0, 0))
                {
