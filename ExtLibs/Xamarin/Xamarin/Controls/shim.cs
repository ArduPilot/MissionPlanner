    ...
    public class Common
    {
        public static GMapMarker getMAVMarker(MAVState MAV)
        {
            ...
            if (MAV.aptype == MAVLink.MAV_TYPE.FIXED_WING)
            {
                ...
                if ((MAV.sysid >= 1) && (MAV.sysid <= 6)) { which = (int)(MAV.sysid - 1); }  //1=black, 2=blue, 3=green,4=yellow,5=orange,6=red
                if ((MAV.sysid >= 11) && (MAV.sysid <= 16)) { which = (int)(MAV.sysid - 11); }  //1=black, 2=blue, 3=green,4=yellow,5=orange,6=red
                if ((MAV.sysid >= 101) && (MAV.sysid <= 106)) { which = (int)(MAV.sysid - 101); }  //1=black, 2=blue, 3=green,4=yellow,5=orange,6=red
                    ...
                    MAV.cs.radius * CurrentState.multiplierdist)
                {
                    ToolTipText = MAV.cs.alt.ToString("0") + CurrentState.AltUnit + " | " + (int)MAV.cs.airspeed +
                                  CurrentState.SpeedUnit + " | id:" + MAV.sysid,
