        ...
        public static Dictionary<int, string> Translate<T>(string defaultText, bool checkPrivate, bool sorting)
        {
            ...
            foreach (
                FieldInfo fieldInfo in
                    typeof(T).GetFields(BindingFlags.Static | BindingFlags.GetField | BindingFlags.Public))
            {
                bool add = true;
            ...
            if (sorting)
            {
                foreach (var x in tempTypes.OrderBy(x => x.Value).ToDictionary(x => x.Key, x => x.Value))
                    types.Add(x.Key, x.Value);
            }
            else
            {
                foreach (var x in tempTypes.ToDictionary(x => x.Key, x => x.Value))
                    types.Add(x.Key, x.Value);
            }
            return types;
        }
