...
namespace DroneCAN
{
    public partial class DroneCAN {
        static void encode_com_xacti_GnssStatus(com_xacti_GnssStatus msg, dronecan_serializer_chunk_cb_ptr_t chunk_cb, object ctx, bool fdcan) {
            uint8_t[] buffer = new uint8_t[8];
            _encode_com_xacti_GnssStatus(buffer, msg, chunk_cb, ctx, !fdcan);
        }

        static uint32_t decode_com_xacti_GnssStatus(CanardRxTransfer transfer, com_xacti_GnssStatus msg, bool fdcan) {
            uint32_t bit_ofs = 0;
            _decode_com_xacti_GnssStatus(transfer, ref bit_ofs, msg, !fdcan);
            return (bit_ofs+7)/8;
        }

        static void _encode_com_xacti_GnssStatus(uint8_t[] buffer, com_xacti_GnssStatus msg, dronecan_serializer_chunk_cb_ptr_t chunk_cb, object ctx, bool tao) {
            memset(buffer,0,8);
        ...
        static void _decode_com_xacti_GnssStatus(CanardRxTransfer transfer,ref uint32_t bit_ofs, com_xacti_GnssStatus msg, bool tao) {
