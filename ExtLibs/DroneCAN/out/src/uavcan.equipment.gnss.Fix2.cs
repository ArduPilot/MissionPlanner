...
namespace DroneCAN
{
    public partial class DroneCAN {
        ...
        {
            public static void encode_uavcan_equipment_gnss_Fix2(uavcan_equipment_gnss_Fix2 msg, dronecan_serializer_chunk_cb_ptr_t chunk_cb, object ctx, bool fdcan) {
                uint8_t[] buffer = new uint8_t[8];
                _encode_uavcan_equipment_gnss_Fix2(buffer, msg, chunk_cb, ctx, !fdcan);
            }

            public static uint32_t decode_uavcan_equipment_gnss_Fix2(CanardRxTransfer transfer, uavcan_equipment_gnss_Fix2 msg, bool fdcan) {
                uint32_t bit_ofs = 0;
                _decode_uavcan_equipment_gnss_Fix2(transfer, ref bit_ofs, msg, !fdcan);
                return (bit_ofs+7)/8;
            }

            internal static void _encode_uavcan_equipment_gnss_Fix2(uint8_t[] buffer, uavcan_equipment_gnss_Fix2 msg, dronecan_serializer_chunk_cb_ptr_t chunk_cb, object ctx, bool tao) {
                uavcan_Timestamp._encode_uavcan_Timestamp(buffer, msg.timestamp, chunk_cb, ctx, false);
                uavcan_Timestamp._encode_uavcan_Timestamp(buffer, msg.gnss_timestamp, chunk_cb, ctx, false);
                memset(buffer,0,8);
                ...
                for (int i=0; i < msg.ecef_position_velocity_len; i++) {
                        uavcan_equipment_gnss_ECEFPositionVelocity._encode_uavcan_equipment_gnss_ECEFPositionVelocity(buffer, msg.ecef_position_velocity[i], chunk_cb, ctx, false);
                }
            }

            internal static void _decode_uavcan_equipment_gnss_Fix2(CanardRxTransfer transfer,ref uint32_t bit_ofs, uavcan_equipment_gnss_Fix2 msg, bool tao) {

                uavcan_Timestamp._decode_uavcan_Timestamp(transfer, ref bit_ofs, msg.timestamp, false);

                uavcan_Timestamp._decode_uavcan_Timestamp(transfer, ref bit_ofs, msg.gnss_timestamp, false);
                ...
                if (tao) {
                    ...
                    while (((transfer.payload_len*8)-bit_ofs) > 0) {
                        ...
                        uavcan_equipment_gnss_ECEFPositionVelocity._decode_uavcan_equipment_gnss_ECEFPositionVelocity(transfer, ref bit_ofs, temp[msg.ecef_position_velocity_len - 1], false);
                    }
                    ...
                    for (int i=0; i < msg.ecef_position_velocity_len; i++) {
                        uavcan_equipment_gnss_ECEFPositionVelocity._decode_uavcan_equipment_gnss_ECEFPositionVelocity(transfer, ref bit_ofs, msg.ecef_position_velocity[i], false);
                    }
                }
