...
namespace DroneCAN
{
    public partial class DroneCAN {
        static void encode_uavcan_protocol_file_Write_req(uavcan_protocol_file_Write_req msg, dronecan_serializer_chunk_cb_ptr_t chunk_cb, object ctx, bool fdcan) {
            uint8_t[] buffer = new uint8_t[8];
            _encode_uavcan_protocol_file_Write_req(buffer, msg, chunk_cb, ctx, !fdcan);
        }

        static uint32_t decode_uavcan_protocol_file_Write_req(CanardRxTransfer transfer, uavcan_protocol_file_Write_req msg, bool fdcan) {
            uint32_t bit_ofs = 0;
            _decode_uavcan_protocol_file_Write_req(transfer, ref bit_ofs, msg, !fdcan);
            return (bit_ofs+7)/8;
        }

        static void _encode_uavcan_protocol_file_Write_req(uint8_t[] buffer, uavcan_protocol_file_Write_req msg, dronecan_serializer_chunk_cb_ptr_t chunk_cb, object ctx, bool tao) {
            memset(buffer,0,8);
