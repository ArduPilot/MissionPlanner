...
namespace DroneCAN
{
    public partial class DroneCAN {
        static void encode_dronecan_remoteid_OperatorID(dronecan_remoteid_OperatorID msg, dronecan_serializer_chunk_cb_ptr_t chunk_cb, object ctx, bool fdcan) {
            uint8_t[] buffer = new uint8_t[8];
            _encode_dronecan_remoteid_OperatorID(buffer, msg, chunk_cb, ctx, !fdcan);
        }

        static uint32_t decode_dronecan_remoteid_OperatorID(CanardRxTransfer transfer, dronecan_remoteid_OperatorID msg, bool fdcan) {
            uint32_t bit_ofs = 0;
