...
namespace DroneCAN
{
    public partial class DroneCAN {
        static void encode_dronecan_remoteid_OperatorID(dronecan_remoteid_OperatorID msg, dronecan_serializer_chunk_cb_ptr_t chunk_cb, object ctx) {
            uint8_t[] buffer = new uint8_t[8];
            _encode_dronecan_remoteid_OperatorID(buffer, msg, chunk_cb, ctx, true);
        }

        static uint32_t decode_dronecan_remoteid_OperatorID(CanardRxTransfer transfer, dronecan_remoteid_OperatorID msg) {
            uint32_t bit_ofs = 0;
            _decode_dronecan_remoteid_OperatorID(transfer, ref bit_ofs, msg, true);
            return (bit_ofs+7)/8;
        }

        static void _encode_dronecan_remoteid_OperatorID(uint8_t[] buffer, dronecan_remoteid_OperatorID msg, dronecan_serializer_chunk_cb_ptr_t chunk_cb, object ctx, bool tao) {
            memset(buffer,0,8);
        ...
        static void _decode_dronecan_remoteid_OperatorID(CanardRxTransfer transfer,ref uint32_t bit_ofs, dronecan_remoteid_OperatorID msg, bool tao) {
