...
namespace DroneCAN
{
    public partial class DroneCAN {
        static void encode_uavcan_equipment_indication_SingleLightCommand(uavcan_equipment_indication_SingleLightCommand msg, dronecan_serializer_chunk_cb_ptr_t chunk_cb, object ctx, bool fdcan) {
            uint8_t[] buffer = new uint8_t[8];
            _encode_uavcan_equipment_indication_SingleLightCommand(buffer, msg, chunk_cb, ctx, !fdcan);
        }

        static uint32_t decode_uavcan_equipment_indication_SingleLightCommand(CanardRxTransfer transfer, uavcan_equipment_indication_SingleLightCommand msg, bool fdcan) {
            uint32_t bit_ofs = 0;
            _decode_uavcan_equipment_indication_SingleLightCommand(transfer, ref bit_ofs, msg, !fdcan);
            return (bit_ofs+7)/8;
        }

        static void _encode_uavcan_equipment_indication_SingleLightCommand(uint8_t[] buffer, uavcan_equipment_indication_SingleLightCommand msg, dronecan_serializer_chunk_cb_ptr_t chunk_cb, object ctx, bool tao) {
            memset(buffer,0,8);
