...
namespace DroneCAN
{
    public partial class DroneCAN {
        static void encode_ardupilot_gnss_RelPosHeading(ardupilot_gnss_RelPosHeading msg, dronecan_serializer_chunk_cb_ptr_t chunk_cb, object ctx, bool fdcan) {
            uint8_t[] buffer = new uint8_t[8];
            _encode_ardupilot_gnss_RelPosHeading(buffer, msg, chunk_cb, ctx, !fdcan);
        }

        static uint32_t decode_ardupilot_gnss_RelPosHeading(CanardRxTransfer transfer, ardupilot_gnss_RelPosHeading msg, bool fdcan) {
            uint32_t bit_ofs = 0;
