...
namespace DroneCAN
{
    public partial class DroneCAN {
        static void encode_dronecan_protocol_CanStats(dronecan_protocol_CanStats msg, dronecan_serializer_chunk_cb_ptr_t chunk_cb, object ctx, bool fdcan) {
            uint8_t[] buffer = new uint8_t[8];
            _encode_dronecan_protocol_CanStats(buffer, msg, chunk_cb, ctx, !fdcan);
        }

        static uint32_t decode_dronecan_protocol_CanStats(CanardRxTransfer transfer, dronecan_protocol_CanStats msg, bool fdcan) {
            uint32_t bit_ofs = 0;
            _decode_dronecan_protocol_CanStats(transfer, ref bit_ofs, msg, !fdcan);
            return (bit_ofs+7)/8;
        }

        static void _encode_dronecan_protocol_CanStats(uint8_t[] buffer, dronecan_protocol_CanStats msg, dronecan_serializer_chunk_cb_ptr_t chunk_cb, object ctx, bool tao) {
            memset(buffer,0,8);
        ...
        static void _decode_dronecan_protocol_CanStats(CanardRxTransfer transfer,ref uint32_t bit_ofs, dronecan_protocol_CanStats msg, bool tao) {
