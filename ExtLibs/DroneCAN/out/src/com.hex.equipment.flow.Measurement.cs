...
namespace DroneCAN
{
    public partial class DroneCAN {
        static void encode_com_hex_equipment_flow_Measurement(com_hex_equipment_flow_Measurement msg, dronecan_serializer_chunk_cb_ptr_t chunk_cb, object ctx, bool fdcan) {
            uint8_t[] buffer = new uint8_t[8];
            _encode_com_hex_equipment_flow_Measurement(buffer, msg, chunk_cb, ctx, !fdcan);
        }

        static uint32_t decode_com_hex_equipment_flow_Measurement(CanardRxTransfer transfer, com_hex_equipment_flow_Measurement msg, bool fdcan) {
            uint32_t bit_ofs = 0;
