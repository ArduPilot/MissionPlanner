...
namespace DroneCAN
{
    public partial class DroneCAN {
        static void encode_ardupilot_indication_Button(ardupilot_indication_Button msg, dronecan_serializer_chunk_cb_ptr_t chunk_cb, object ctx, bool fdcan) {
            uint8_t[] buffer = new uint8_t[8];
            _encode_ardupilot_indication_Button(buffer, msg, chunk_cb, ctx, !fdcan);
        }

        static uint32_t decode_ardupilot_indication_Button(CanardRxTransfer transfer, ardupilot_indication_Button msg, bool fdcan) {
            uint32_t bit_ofs = 0;
            _decode_ardupilot_indication_Button(transfer, ref bit_ofs, msg, !fdcan);
            return (bit_ofs+7)/8;
        }

        static void _encode_ardupilot_indication_Button(uint8_t[] buffer, ardupilot_indication_Button msg, dronecan_serializer_chunk_cb_ptr_t chunk_cb, object ctx, bool tao) {
            memset(buffer,0,8);
