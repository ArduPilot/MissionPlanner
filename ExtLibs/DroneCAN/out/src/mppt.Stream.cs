...
namespace DroneCAN
{
    public partial class DroneCAN {
        static void encode_mppt_Stream(mppt_Stream msg, dronecan_serializer_chunk_cb_ptr_t chunk_cb, object ctx, bool fdcan) {
            uint8_t[] buffer = new uint8_t[8];
            _encode_mppt_Stream(buffer, msg, chunk_cb, ctx, !fdcan);
        }

        static uint32_t decode_mppt_Stream(CanardRxTransfer transfer, mppt_Stream msg, bool fdcan) {
            uint32_t bit_ofs = 0;
