...
namespace DroneCAN
{
    public partial class DroneCAN {
        static void encode_uavcan_equipment_gnss_Auxiliary(uavcan_equipment_gnss_Auxiliary msg, dronecan_serializer_chunk_cb_ptr_t chunk_cb, object ctx, bool fdcan) {
            uint8_t[] buffer = new uint8_t[8];
            _encode_uavcan_equipment_gnss_Auxiliary(buffer, msg, chunk_cb, ctx, !fdcan);
        }

        static uint32_t decode_uavcan_equipment_gnss_Auxiliary(CanardRxTransfer transfer, uavcan_equipment_gnss_Auxiliary msg, bool fdcan) {
            uint32_t bit_ofs = 0;
