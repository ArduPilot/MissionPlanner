...
@(ind)    canardDecodeScalar(transfer, bit_ofs, @(field.type.bitlen), @('true' if dronecan_type_is_signed(field.type) else 'false'), ref float16_val);
@(ind)    msg.@(field.name) = canardConvertFloat16ToNativeFloat(float16_val);
@(ind)}
@[        else]@
@[      if msg_union]@
@(ind)canardDecodeScalar(transfer, bit_ofs, @(field.type.bitlen), @('true' if dronecan_type_is_signed(field.type) else 'false'), ref msg.union.@(field.name));
@[        else]@
@(ind)canardDecodeScalar(transfer, bit_ofs, @(field.type.bitlen), @('true' if dronecan_type_is_signed(field.type) else 'false'), ref msg.@(field.name));
@[        end if]@
@[        end if]@
...
@(ind)var temp = new List<@(dronecan_type_to_ctype(field.type.value_type))>();
...
@{indent += 1}@{ind = '    '*indent}@
@[              end if]@
@(ind)msg.@('union.' if msg_union else '')@(field.name) = new @(dronecan_type_to_ctype(field.type.value_type))[msg.@('union.' if msg_union else '')@(field.name)_len];
@(ind)for (int i=0; i < msg.@('union.' if msg_union else '')@(field.name)_len; i++) {
@[        else]@
@(ind)for (int i=0; i < @(field.type.max_size); i++) {
@[        end if]@
...
@(ind){
@(ind)    uint16_t float16_val = 0;
@(ind)    canardDecodeScalar(transfer, bit_ofs, @(field.type.value_type.bitlen), @('true' if dronecan_type_is_signed(field.type.value_type) else 'false'), ref float16_val);
@(ind)    msg.@(field.name)[i] = canardConvertFloat16ToNativeFloat(float16_val);
@(ind)}
@[          else]@
@(ind)canardDecodeScalar(transfer, bit_ofs, @(field.type.value_type.bitlen), @('true' if dronecan_type_is_signed(field.type.value_type) else 'false'), ref msg.@('union.' if msg_union else '')@(field.name)[i]);
@[          end if]@
